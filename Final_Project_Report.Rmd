...
min.X <- min(AQ_data$Lon)
min.Y <- min(AQ_data$Lat)
max.X <- max(AQ_data$Lon)
max.Y <- max(AQ_data$Lat)
x.range <- max.X - min.X
y.range <- max.Y - min.Y
...
```
```{r}
# Intersect interpolated emissions with Toronto census information
idw_intersect <- st_inter(torontoDAs.centroids, P.idw.sf)
```

```{r}
# Plot the data
ggplot(data = idw_intersect) + geom_sf(aes(colour = P))
```
...
toDA.centroids.idw <- mutate(torontoDAs.centroids, 
                             PM25 = kpoint.10$P)
