...
krig_pred_var <- function(observations, targets, vgm_model, trend = "None"){
  ...
  V.kriged <- krige(trend_model,
                    ...
                    targets, 
                    variogram_v.t)
  return(c("mean" = mean(V.kriged$var1.var), "sd" = sd(V.kriged$var1.var)))
}
```
...
Projecting:
```{r}
#st_transform(torontoDAs.demographics.sf)
```


```{r}
#summary(torontoCTs.demographics.sf)
ggplot(torontoCTs.demographics.sf) + geom_sf(aes(fill = median_hh_income)) + labs(fill = "Median Household Income")
ggplot(torontoCTs.demographics.sf) + geom_sf(aes(fill = population_density)) + labs(fill = "Population density per square km")
ggplot(torontoDAs.demographics.sf) + geom_sf(aes(fill = median_hh_income)) + labs(fill = "Median Household Income")
```
