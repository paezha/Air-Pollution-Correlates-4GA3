...
AQ_data.sf <- AQ_data |> 
  st_as_sf(coords = c("Lon", "Lat"), 
           remove = "FALSE") # Keep Lon and Lat columns
```
...
kpoint.5 <- kpointmean(source_xy = AQ_data.sf, 
                       target_xy = torontoDAs.centroids, 
                       z = P, 
                       k = 10) |>
  rename(P = z)
```
Using K-point means interpolation
...
```{r}
# Categorize X and Y in separate columns
...
min.X <- min(AQ_data.sf$X)
min.Y <- min(AQ_data.sf$Y)
max.X <- max(AQ_data.sf$X)
max.Y <- max(AQ_data.sf$Y)
...
AQ_data.ppp <- as.ppp(X = AQ_data.sf[,c(4, 12:13)], W = AQ.owin)
```
...
```{r}
z_p.idw1 <- idw(AQ_data.ppp, power = 1)
```



Kriging (currently not working)

```{r}
# Trying kriging function again
# toronto_int <- krig_pred_var(AQ_data.sf, torontoDAs.centroids, "Sph", "Cubic")
```
